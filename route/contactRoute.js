...
router.post('/contact', (req, res) => {
    ...
    smtpTransporter.sendMail(mailOptions, (error) => {
        try {
            if (error) return res.status(400).json({ msg: 'please fill all the feilds Carefully' })
            res.status(200).json({ msg: 'Thank you  for  Contacting Sandeep' })
        } catch (error) {
